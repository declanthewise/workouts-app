...
A running workout has no page of its own — it plays out *inside* the home rows. `workoutActive` (boolean) drives `useWorkoutSession` (`src/useWorkoutSession.js`), the session engine: it freezes a snapshot of `activeLevels` at start (so mid-session equipment changes can't scramble the queue), tracks phase/queue/rest state, and exposes `rowState(treeIdx)`. `App.jsx` feeds each `ProgressionRow` a `session` prop built from it: rows outside the current phase dim to view-only, the current pair's rows stay live — tapping the active tile logs a set (set dots replace the level ticks during a session), and the just-finished tile turns into a rest countdown (tap = skip 10s). During a session the header's Start button becomes a red Stop workout button; there is no pause — navigating between views leaves the clock running. Stopping mid-session opens a confirm dialog (`StopConfirm.jsx`) since session progress isn't persisted; once the workout is complete, Stop ends immediately. Completion renders a banner at the top of the rows column whose End button (or Stop workout anytime) ends the session and restores normal rows.
